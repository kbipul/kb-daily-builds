...
import { diagnose, optimize, rollup, totalTokens, validateStack } from '../engine/cache';
...
describe('README claims', () => {
  ...
  it('the RAG preset gets more than 60% cheaper with markers alone', () => {
    const rag = byName('Enterprise RAG');
    const now = rollup(rag, fable, '5m').perMonth;
    const fixed = rollup(optimize(rag), fable, '5m').perMonth;
    expect((now - fixed) / now).toBeGreaterThan(0.6);
  });
